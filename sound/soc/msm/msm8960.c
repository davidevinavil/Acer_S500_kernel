...
static int msm8960_configure_speaker_amp_gpios(void)
{
	int ret = 0;
...
#ifdef CONFIG_MACH_ACER_A9
	snd_jack_set_key(button_jack.jack, SND_JACK_BTN_0, KEY_MEDIA);
	snd_jack_set_key(button_jack.jack, SND_JACK_BTN_1, KEY_VOLUMEUP);
	snd_jack_set_key(button_jack.jack, SND_JACK_BTN_2, KEY_VOLUMEDOWN);
#endif
...
#ifdef CONFIG_MACH_ACER_A9
	ret = msm8960_configure_speaker_amp_gpios();
	if (ret) {
		pr_err("%s Fail to configure speaker amp gpios\n", __func__);
	}
#endif
