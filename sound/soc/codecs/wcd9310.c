...
static int wcd9310_open(struct inode *inode, struct file *file);
static int wcd9310_close(struct inode *inode, struct file *file);
static long wcd9310_ioctl(struct file *file, unsigned int cmd, unsigned long arg);
unsigned int wcd9310_get_chip_version(void);
#endif

#ifdef CONFIG_ACER_HEADSET
void acer_headset_mic_bias(int state);
void acer_headset_jack_report(int headset_type);
void acer_headset_control(int plug_in);
#endif
...
static const struct file_operations wcd9310_fops = {
	.owner			= THIS_MODULE,
	.open			= wcd9310_open,
	.release		= wcd9310_close,
	.unlocked_ioctl	= wcd9310_ioctl,
};

static struct miscdevice wcd9310_dev = {
	.minor	= MISC_DYNAMIC_MINOR,
	.name	= WCD9310_DRIVER_NAME,
	.fops	= &wcd9310_fops,
};
...
static ssize_t codec_show(struct kobject *kobj, struct kobj_attribute *attr, char * buf)
{
	...
	ver = wcd9310_get_chip_version();
	s += sprintf(s, "wcd9310, version=%i\n", ver);
	return (s - buf);
}
...
static ssize_t dolby_show(struct kobject *kobj, struct kobj_attribute *attr, char * buf)
{
	...
	s += sprintf(s, "Dolby Mobile3 plus\n");
	return (s - buf);
}
...
static long wcd9310_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	pr_debug("[WCD9310] wcd9310 ioctl \n");
	if (_IOC_TYPE(cmd) != WCD9310_IOCTL_MAGIC) {
		pr_err("[WCD9310] IOCTL: cmd magic type error\n");
		return -ENOTTY;
	}

	if (_IOC_NR(cmd) > IOC_MAXNR) {
		pr_err("[WCD9310] IOCTL: cmd number error\n");
		return -ENOTTY;
	}
	...
	if (err) {
		pr_err("[WCD9310] IOCTL: cmd access right error\n");
		return -EFAULT;
	}

	switch(cmd) {
	case WCD9310_SET_CODEC: {
		unsigned int reg;
		unsigned int value;
		struct wcd9310_codec_info codec_info;

		printk("AUDIO_SET_CODEC...\r\n");

		if (copy_from_user(&codec_info, (void __user *)arg, sizeof(struct wcd9310_codec_info)))
			return -1;
	...
	case WCD9310_GET_CODEC: {
		...
		if (copy_to_user((void __user *) arg, &codec_info, sizeof(struct wcd9310_codec_info)))
			return -1;
...
unsigned int wcd9310_get_chip_version(void)
{
	...
	value = snd_soc_read(acer_codec, TABLA_A_CHIP_VERSION);
...
#ifdef CONFIG_ACER_HEADSET
struct tabla_priv *acer_tabla_priv;
static void tabla_codec_enable_bandgap(struct snd_soc_codec *codec, enum tabla_bandgap_type choice);
...
{
	if (state) {
		tabla_turn_onoff_override(acer_codec, true);
		tabla_codec_setup_hs_polling(acer_codec);
		tabla_turn_onoff_override(acer_codec, false);
	} else {
		tabla_codec_enable_bandgap(acer_codec, TABLA_BANDGAP_OFF);
		tabla_codec_disable_clock_block(acer_codec);
	}
}
...
void acer_headset_jack_report(int headset_type)
{
	switch(headset_type) {
	case 0:
		snd_soc_jack_report_no_dapm(acer_tabla_priv->mbhc_cfg.headset_jack,
									0,
									TABLA_JACK_MASK);
		break;
...
void press_headset_button(int button_num)
{
	int button_press = 0;
	u64 adc_reply;
	struct tabla_priv *tabla = snd_soc_codec_get_drvdata(acer_codec);
	...
	case 0:
		...
		break;
	case 1:
		button_press = SND_JACK_BTN_1;
		break;
	case 2:
		button_press = SND_JACK_BTN_2;
		break;
	}

	if (button_press == SND_JACK_BTN_0) {
		snd_soc_jack_report(tabla->mbhc_cfg.button_jack, button_press, button_press);
		snd_soc_jack_report(tabla->mbhc_cfg.button_jack, 0, button_press);
	} else {
		snd_soc_jack_report(tabla->mbhc_cfg.button_jack, button_press, button_press);
		do {
			mdelay(10);
			get_mpp_hs_mic_adc(&adc_reply);
		} while ((adc_reply < 0x9000) && (!gpio_get_value(GPIO_HS_BUTT)));
		snd_soc_jack_report(tabla->mbhc_cfg.button_jack, 0, button_press);
	}
}
#endif
...
#ifdef CONFIG_MACH_ACER_A9
	snd_soc_update_bits(codec, tx_mux_ctl_reg, 0x30, 0x20);
#else
...
static void tabla_codec_detect_plug_type(struct snd_soc_codec *codec)
{
	...
	acer_headset_mic_bias(1);
	while (count < 3) {
			get_mpp_hs_mic_adc(&buff);
			mdelay(50);
			count++;
	}
...
{
	if (plug_in) {
		tabla_hs_insert_irq(0, acer_tabla_priv);
	} else {
		tabla_hs_remove_irq(0, acer_tabla_priv);
	}
}
#endif
...
#ifdef CONFIG_MACH_ACER_A9
	{TABLA_A_RX_LINE_1_GAIN, 0x50, 0x50},
#else
	{TABLA_A_RX_LINE_1_GAIN, 0x10, 0x10},
#endif
	{TABLA_A_RX_LINE_2_GAIN, 0x10, 0x10},
#ifdef CONFIG_MACH_ACER_A9
	{TABLA_A_RX_LINE_3_GAIN, 0x50, 0x50},
#else
...
#ifdef CONFIG_MACH_ACER_A9
	acer_codec = codec;
	if (misc_register(&wcd9310_dev)) {
		pr_err("wcd9310_probe: wcd9310_dev register failed\n");
		return -ENOMEM;
	}
#endif
...
#ifdef CONFIG_ACER_HEADSET
	acer_tabla_priv = tabla;
	acer_hs_init();
#endif

#ifdef CONFIG_ACER_HEADSET_BUTT
	ret = acer_hs_butt_init();
#endif
