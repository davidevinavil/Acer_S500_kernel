...
static void acer_headset_delay_butt_work(struct work_struct *work)
{
	pr_debug("Enable headset button !!!\n");
#ifdef CONFIG_ACER_HEADSET_BUTT
	set_hs_state(true);
#endif
}
...
static int hsedbias2_vreg_enable(void)
{
	...
	rc = regulator_set_voltage(hsed_vdd, 2950000, 2950000);
	if (rc) {
...
static int hsedbias2_vreg_disable(void)
{
	...
	rc = regulator_set_voltage(hsed_vdd, 0, 2950000);
	if (rc) {
		pr_err("%s: regulator_set_voltage"
				"failed rc =%d\n", __func__, rc);
		goto error_vdd;
...
static void acer_headset_delay_set_work(struct work_struct *work)
{
	...
	pr_debug("acer hs detect state = %d...\n", hs_disable);

	if (!hs_disable) {
		if (acer_board_id <= HW_ID_DVT2) {
			acer_headset_control(1);
		} else {
			// HSED_BIAS2 enable
			hsedbias2_vreg_enable();
			pm8xxx_hsed_bias_control(PM8XXX_HSED_BIAS1, 1);
		...
		if (acer_board_id <= HW_ID_DVT2) {
			...
			pm8xxx_hsed_bias_control(PM8XXX_HSED_BIAS1, 0);
...
int acer_hs_init(void)
{
	...
	if (acer_board_id > HW_ID_DVT2) {
		hsed_vdd = regulator_get(NULL, "8921_l6");
		if (IS_ERR(hsed_vdd)) {
