...
int get_mpp_hs_mic_adc(u64 *val)
{
	...
	if (!rc)
		pr_debug("ADC MPP value chan:%d raw:%x measurement:%lld physical:%lld\n",
...
static void rpc_call_work(struct work_struct *work)
{
	...
	if (hs_state) {
		if (!gpio_get_value(hr->butt)) {
			pr_debug("HS button is pressing...\n");
			while (count < 3) {
			...
			if (acer_board_id >= HW_ID_PVT1) {
				...
				if (adc_reply >= 0x6000 && adc_reply < 0x6500) {
					pr_debug("HS button : play/pause function...\n");
					press_headset_button(0);
				} else if (adc_reply >= 0x6500 && adc_reply < 0x7000) {
					pr_debug("HS button : volume up function...\n");
					press_headset_button(1);
				} else if (adc_reply >= 0x7000 && adc_reply < 0x8000) {
					pr_debug("HS button : volume down function...\n");
					press_headset_button(2);
				}
			}
