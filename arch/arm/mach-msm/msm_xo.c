...
void debug_dump_XO_status(void)
{
	...
	pxo = &msm_xo_sources[MSM_XO_PXO];
	cxo = &msm_xo_sources[MSM_XO_CXO];
	...
	list_for_each_entry(voter, &pxo->voters, list)
		if (voter->mode != MSM_XO_MODE_OFF)
			goto dump_status;

	list_for_each_entry(voter, &cxo->voters, list)
		if (voter->mode != MSM_XO_MODE_OFF)
			goto dump_status;

	goto dump_status_exit;
...
dump_status:
	...
	msm_xo_dump_xo_print(&msm_xo_sources[MSM_XO_PXO], "PXO during sleep");
	msm_xo_dump_xo_print(&msm_xo_sources[MSM_XO_CXO], "CXO");
