...
int NFC_DrvOp(enum nfc_drv_op_e eNfcDrvTestItem, void * pvInOutBuf)
{
	...
	pr_info("[NFC_DrvTest] NFC_DrvOp() enter, op = 0x%02x\n", eNfcDrvTestItem);
	...
	switch(eNfcDrvTestItem)
	{
		...
		case check_mode_status:
			{
				if (TRUE == g_ptNfcPrjInfo->bNfcEnabled)
				{
					if (gpio_get_value(NXP_PN544_FW_DW))
						pr_info ("[NFC_TEST] pn544 in <Download> mode\n");
					else
						NFC_CheckSet_ActiveStandbyMode(TRUE, nfc_none);
				}
				else /* (FALSE == g_ptPn544Info->tNfc_pri_info.bNfcEnabled) */
					pr_info ("[NFC_TEST] pn544 in <Disable> mode\n");
			}
			break;

		case enable_nfc_hci_mode:
			{
		...
		case act_read_card_id:
			{
				pu8Buf = (PU8)pvInOutBuf;

				if (pu8Buf == NULL || (sizeof(pu8Buf)/sizeof(pu8Buf[0])) < 4)
					iRet = FAILURE;
				else
