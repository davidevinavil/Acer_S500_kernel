...
#ifdef CONFIG_MACH_ACER_A9
#define DRIVER_VERSION			"3.1.6"
#define BATTERY_MISSING_ENABLE
#define BATTERY_DEBOUNCE_MS 1000
#define USB_CHG_MA  500  //500mA
#define AC_CHG_MA  1100  //1100mA
#endif
...
struct pm8921_chg_chip {
	...
	int				last_batt_cap;
	struct delayed_work		batt_missing_work;
#endif
};
...
//Additional A9 USB charge type
static bool is_acer_fast_charge = false;
static bool is_unknow_charge = false;

//dev-info_battery sysfs
static struct kobject *pm8921_chg_dbg_kobj;
static bool is_vbus_disable = false;
static bool is_chg_disable = false;
bool is_psy_debug_on = false;
static int vbus_draw_current = 0;
extern int last_calc_fcc;
extern int last_calc_pc;
extern int last_calc_soc;
extern int last_calc_ruc;

//Battery missing debounce
static bool is_battery_missing = false;
static bool is_battery_hot = false;
static bool is_battery_cold = false;
static void batt_missing_worker(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct pm8921_chg_chip *chip = container_of(dwork,
				struct pm8921_chg_chip, batt_missing_work);

	if (!get_prop_batt_present(chip)) {
		pr_info("Battery missing!!\n");
		is_battery_missing = true;
		power_supply_changed(&chip->usb_psy);
		power_supply_changed(&chip->dc_psy);
		power_supply_changed(&chip->batt_psy);
	} else {
		int batt_temp = get_prop_batt_temp(chip);
		pr_info("Battery temperture is %s%d.%ddegC\n",
			(batt_temp<0 && batt_temp>-10) ? "-" : "",
			batt_temp/10, batt_temp>=0 ? batt_temp%10 : -batt_temp%10);
		if (batt_temp >= 480) {  //Hot
			is_battery_hot = true;
		} else if (batt_temp <= 20) {  //Cold
			is_battery_cold = true;
		} else {  //Normal
			if (is_chg_disable) {
				if (is_usb_chg_plugged_in(chip)) {
					pm8921_charger_enable(false);
					pr_info("disable charger again\n");
				}
			}
			is_battery_hot = false;
			is_battery_cold = false;
		}
		is_battery_missing = false;
		power_supply_changed(&chip->batt_psy);
	}
}

//Gauge functions
static struct pmic_battery_gauge *pm8921_batt_gauge;

void pm8921_battery_gauge_register(struct pmic_battery_gauge *batt_gauge)
{
	if (pm8921_batt_gauge) {
		pm8921_batt_gauge = batt_gauge;
...
EXPORT_SYMBOL(pm8921_battery_gauge_register);

void pm8921_battery_gauge_unregister(struct pmic_battery_gauge *batt_gauge)
{
	pm8921_batt_gauge = NULL;
	pr_info("done\n");
}
...
{
	if (pm8921_batt_gauge)
		return gauge_it_enable(enable);
	else
		return -1;
}
EXPORT_SYMBOL(pm8921_gauge_it_enable);

extern int gauge_device_type(void);
int pm8921_gauge_device_type(void)
{
	if (pm8921_batt_gauge)
		return gauge_device_type();
	else
		return -1;
}
EXPORT_SYMBOL(pm8921_gauge_device_type);

extern int gauge_firmware_update(bool it_enable);
int pm8921_gauge_firmware_update(bool it_enable)
{
	if (pm8921_batt_gauge)
		return gauge_firmware_update(it_enable);
	else
		return -1;
}
				  ...
				  union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (charging_disabled)
			return 0;
#ifdef CONFIG_MACH_ACER_A9
#ifdef BATTERY_MISSING_ENABLE
		//Battery missing
		if (is_battery_missing)
			return 0;
#endif
		if (is_unknow_charge) {
			val->intval = 0;
		...
		if (usb_target_ma <= USB_WALL_THRESHOLD_MA) {
			...
			if (is_psy_debug_on) {
				if (psp == POWER_SUPPLY_PROP_ONLINE)
					pr_info("Online=%d usb_target_ma=%d", val->intval, usb_target_ma);
			}
		}
#else
		if (usb_target_ma <= USB_WALL_THRESHOLD_MA)
...
#ifdef CONFIG_MACH_ACER_A9
	if (pm8921_batt_gauge)
		return pm8921_batt_gauge->get_battery_mvolts() * 1000;
#endif
...
#ifdef CONFIG_MACH_ACER_A9
	if (pm8921_batt_gauge)
		return pm8921_batt_gauge->is_battery_present();
#endif
...
static bool is_battery_full = false;
static bool is_fake_enbable = false;
static int fake_cap = 50;
static int soc_offset_eoc = 0, soc_offset_cur = 0;
static int soc_check(struct pm8921_chg_chip *chip, int soc)
{
	int percent_soc = soc;
	int offset_thr_soc = 100 - soc_offset_eoc - 3; //3% buffer

	//Remap battery capacity
	if (soc > offset_thr_soc) {
		if (soc_offset_cur > soc_offset_eoc)
			soc_offset_cur = soc_offset_eoc;
	} else if (soc > 20 && soc <= offset_thr_soc) {
		if (soc_offset_cur > (soc_offset_eoc - (offset_thr_soc-soc)/3)
			&& soc_offset_cur > 0)
			soc_offset_cur = soc_offset_eoc - (offset_thr_soc-soc)/3;
	} else {
		soc_offset_eoc = 0;
		soc_offset_cur = 0;
	}
	percent_soc = soc + soc_offset_cur;
	...
	if (is_usb_chg_plugged_in(chip)) {  //Charging
		//Show 100% when Full
		if (eoc_count == CONSECUTIVE_COUNT) {
			soc_offset_eoc = 100 - soc;
			if (soc_offset_eoc >= 10 || soc_offset_eoc < 0) {
				pr_err("SoC offset abnormal %d%%\n", soc_offset_eoc);
				soc_offset_eoc = 0;
				soc_offset_cur = 0;
			} else {
				percent_soc = 100;
				soc_offset_cur = soc_offset_eoc;
				pr_debug("SoC offset is %d%%\n", soc_offset_cur);
			}
		} else if (eoc_count >= (CONSECUTIVE_COUNT/2) && percent_soc < 99) {
			//Show 99% if battery ready to Full
			percent_soc = 99;
		}
	} else {  //Discharing
		if (soc_offset_eoc > 4 && percent_soc == 100) {
			percent_soc = 99;
			soc_offset_eoc = soc_offset_eoc - 1;
			soc_offset_cur = soc_offset_eoc;
		}
	}
...
static int gauge_soc_check(struct pm8921_chg_chip *chip, int soc)
{
	int percent_soc = soc;

	if (is_usb_chg_plugged_in(chip)) {  //Charging
		if (soc < 100) {
			if (is_battery_full) {  // Battery capacity drop
				soc_offset_eoc = 100 - soc;
				if (soc_offset_eoc > 3) { //96%~0%
					soc_offset_eoc = 3;
				}
				soc_offset_cur = soc_offset_eoc;
			}
		...
		if (soc_offset_cur > 0) {
			if (soc <= 50) {
				soc_offset_cur = soc_offset_eoc - 3;
			} else if (soc <= 70) {
				soc_offset_cur = soc_offset_eoc - 2;
			} else if (soc <= 90) {
				soc_offset_cur = soc_offset_eoc - 1;
			}
			percent_soc = soc + soc_offset_cur;
		} else {
			soc_offset_eoc = 0;
...
static int get_prop_batt_capacity(struct pm8921_chg_chip *chip)
{
	...
	if (pm8921_batt_gauge) {
		percent_soc = pm8921_batt_gauge->get_battery_capacity();
		if (percent_soc <= 10)
			pr_warn("low battery charge = %d%%(Gauge)\n", percent_soc);

		//Check battery capacity for some abnormal condition
		percent_soc = gauge_soc_check(chip, percent_soc);
	} else {
		...
		//Check battery capacity for some abnormal condition
		percent_soc = soc_check(chip, percent_soc);
		mutex_unlock(&chip->mutex_lock);
	}
	chip->last_batt_cap = percent_soc;
	...
	//Fake capacity
	if (is_fake_enbable)
		percent_soc = fake_cap;

	return percent_soc;
}

int pm8921_get_percent_soc(void)
{
	if (!the_chip) {
	...
	return get_prop_batt_capacity(the_chip);
}
EXPORT_SYMBOL_GPL(pm8921_get_percent_soc);
#else
static int get_prop_batt_capacity(struct pm8921_chg_chip *chip)
{
	int percent_soc;
...
#ifdef CONFIG_MACH_ACER_A9
	if (pm8921_batt_gauge)
		return -pm8921_batt_gauge->get_battery_current() * 1000;
#endif
...
#ifdef CONFIG_MACH_ACER_A9
	if (pm8921_batt_gauge)
		return pm8921_batt_gauge->bq27520_std_data(0x12);
#endif
...
static int get_prop_batt_status(struct pm8921_chg_chip *chip)
{
...
#ifdef CONFIG_MACH_ACER_A9
	/* Over temperature without gauge */
	if (fsm_state == FSM_STATE_FAST_CHG_PAUSE_14) {
		if (pm_chg_get_rt_status(chip, USBIN_VALID_IRQ))
			batt_state = POWER_SUPPLY_STATUS_CHARGING;
		if (is_battery_cold || is_battery_hot)
			batt_state = POWER_SUPPLY_STATUS_CHARGING;
		if (pm_chg_get_rt_status(chip, BATT_REMOVED_IRQ))
			batt_state = POWER_SUPPLY_STATUS_NOT_CHARGING;
	}
	/* Stop charging with AC/USB connected */
	...
	/* Still charging, but 100% */
	if (fsm_state == FSM_STATE_FAST_CHG_7 && is_battery_full) {
		batt_state = POWER_SUPPLY_STATUS_FULL;
	}
	if (is_chg_disable && !is_vbus_disable) {
		batt_state = POWER_SUPPLY_STATUS_NOT_CHARGING;
	}
#else
	if (fsm_state == FSM_STATE_ON_CHG_HIGHI_1) {
...
#ifdef CONFIG_MACH_ACER_A9
	if (pm8921_batt_gauge)
		return pm8921_batt_gauge->get_battery_temperature();
#endif
...
#ifdef CONFIG_MACH_ACER_A9
/* MHL calls these to tell us the charge type of vbus */
static bool is_mhl_charge = false;
void pm8921_mhl_charger_vbus_type(enum power_supply_type type)
{
	unsigned long flags;
	int mA = 0;

	pr_debug("Enter MHL charge type=%d\n", type);
	switch (type) {
...
EXPORT_SYMBOL_GPL(pm8921_mhl_charger_vbus_type);
#endif
...
int pm8921_set_usb_power_supply_type(enum power_supply_type type)
{
	...
	if (type <= POWER_SUPPLY_TYPE_USB_ACA) {
		is_acer_fast_charge = false;
		is_unknow_charge = false;
		the_chip->usb_psy.type = type;
	} else {
		if (type == POWER_SUPPLY_TYPE_AFC) {
			is_acer_fast_charge = true;
			__pm8921_charger_vbus_draw(AC_CHG_MA);
		} else if (type == POWER_SUPPLY_TYPE_UNKNOW) {
			is_unknow_charge = true;
			__pm8921_charger_vbus_draw(USB_CHG_MA);
		}
	}
...
static void handle_usb_insertion_removal(struct pm8921_chg_chip *chip)
{
	...
	if (chip->usb_present ^ usb_present) {
		...
		is_chg_disable = false;
		is_vbus_disable = false;
		chip->usb_present = usb_present;
		wake_lock_timeout(&chip->usbin_wake_lock, 5*HZ);
		power_supply_changed(&chip->usb_psy);
		power_supply_changed(&chip->dc_psy);
		power_supply_changed(&chip->batt_psy);
		if (!acer_sku_wi_gauge)
	...
	if (usb_present && is_mhl_charge == 0) {
...
static irqreturn_t batt_inserted_irq_handler(int irq, void *data)
{
	...
	pr_info("battery present=%d", status);
	power_supply_changed(&chip->batt_psy);
	return IRQ_HANDLED;
}
...
static irqreturn_t bat_temp_ok_irq_handler(int irq, void *data)
{
	...
	power_supply_changed(&chip->usb_psy);
	bms_notify_check(chip);
#if defined(CONFIG_MACH_ACER_A9) && defined(BATTERY_MISSING_ENABLE)
	schedule_delayed_work(&chip->batt_missing_work,
						round_jiffies_relative(msecs_to_jiffies
						     (BATTERY_DEBOUNCE_MS)));
#endif
	return IRQ_HANDLED;
...
static void update_heartbeat(struct work_struct *work)
{
	...
	power_supply_changed(&chip->batt_psy);
#ifdef CONFIG_MACH_ACER_A9
	if (chip->last_batt_cap <= 2) {
		schedule_delayed_work(&chip->update_heartbeat_work,
			round_jiffies_relative(msecs_to_jiffies(chip->update_time/2)));
	} else {
		schedule_delayed_work(&chip->update_heartbeat_work,
	...
	if (pm8921_batt_gauge) {
		int temp = pm8921_batt_gauge->get_battery_temperature();
		pr_info("Cap=%d%%(%d) Volt=%dmV Amp=%dmA Temp=%s%d.%ddegC Status=%d\n",
				pm8921_batt_gauge->get_battery_capacity(), soc_offset_cur,
				pm8921_batt_gauge->get_battery_mvolts(),
				-pm8921_batt_gauge->get_battery_current(),
				(temp<0 && temp>-10) ? "-" : "",
				temp/10, temp>=0 ? temp%10 : -temp%10,
				pm8921_batt_gauge->get_battery_status());
	} else {
		int temp = get_prop_batt_temp(chip);
		pr_info("Cap=%d%%(%d) Volt=%dmV Amp=%dmA Temp=%d.%ddegC SoC=%d%%\n",
				chip->last_batt_cap, soc_offset_cur,
				get_prop_battery_uvolts(chip)/1000, get_prop_batt_current(chip)/1000,
				temp/10, temp>=0 ? temp%10 : -temp%10,
				last_calc_soc);
	}
	// Show CPU Temperature
	{
		...
		unsigned long temp0 = 0, temp3 = 0, temp4 = 0;
		int ret = 0;

		tsens_dev.sensor_num = 0;
		ret = tsens_get_temp(&tsens_dev, &temp0);
		if (ret) {
			pr_debug("msm_thermal: Unable to read TSENS sensor %d\n",
					tsens_dev.sensor_num);
		}
		tsens_dev.sensor_num = 3;
		ret = tsens_get_temp(&tsens_dev, &temp3);
		if (ret) {
			pr_debug("msm_thermal: Unable to read TSENS sensor %d\n",
					tsens_dev.sensor_num);
		}
		tsens_dev.sensor_num = 4;
		ret = tsens_get_temp(&tsens_dev, &temp4);
		if (ret) {
			pr_debug("msm_thermal: Unable to read TSENS sensor %d\n",
					tsens_dev.sensor_num);
		}
		pr_info("Sensor0=%d Sensor3=%d Sensor4=%d\n", (int)temp0, (int)temp3, (int)temp4);
	}
#else
	schedule_delayed_work(&chip->update_heartbeat_work,
			      round_jiffies_relative(msecs_to_jiffies
...
static void eoc_worker(struct work_struct *work)
{
	...
	if (pm8921_batt_gauge) {
		if (count == CONSECUTIVE_COUNT) {
			if (pm8921_batt_gauge->get_battery_capacity() < 100) {
				count = 0;
				pr_info("Not full, reset eoc count\n");
			}
		}
	}
	eoc_count = count;
...
#ifndef CONFIG_MACH_ACER_A9
	pr_info("max_bat_chg_current=%dmA\n", chg_current);
#endif
...
static void __devinit determine_initial_state(struct pm8921_chg_chip *chip)
{
	...
	if (is_mhl_charge == 0) {
		...
		if (chip->usb_present) {
			//Set default USB max current to 1100mA first
			rc = pm_chg_iusbmax_set(chip, 0xA);
			if (rc) {
				pr_err("unable to set iusb to 1100mA rc = %d\n", rc);
			}
			...
			//Set default USB max current to 0mA
			rc = pm_chg_iusbmax_set(chip, 0);
			if (rc) {
				pr_err("unable to set iusb to 0mA rc = %d\n", rc);
			}
		}
	} else {
		notify_usb_of_the_plugin_event(0);
	...
	pr_info("usb = %d, dc = %d  batt = %d state=%d\n",
			chip->usb_present,
			chip->dc_present,
			get_prop_batt_present(chip),
			fsm_state);
...
#ifdef CONFIG_MACH_ACER_A9
static int fake_capacity_show(void *data, u64 * val)
{
	*val = fake_cap;
	return 0;
}

static int fake_capacity_store(void *data, u64 val)
{
	u8 temp;
	...
	if (temp >=0 && temp <= 100) {
		fake_cap = temp;
		pr_info("fake battery capacity is set to %d\n", fake_cap);
	} else {
		pr_err("Set wrong capacity!!\n");
	}
	return 0;
}
DEFINE_SIMPLE_ATTRIBUTE(fake_cap_fops, fake_capacity_show,
									fake_capacity_store, "%llu\n");

static int fake_enable_show(void *data, u64 * val)
{
	*val = is_fake_enbable;
	return 0;
}

static int fake_enable_store(void *data, u64 val)
{
	u8 temp;

	temp = (u8) val;
	is_fake_enbable = (bool)temp;
	pr_info("%s fake battery flag\n", is_fake_enbable?"enable":"disable");
	power_supply_changed(&the_chip->batt_psy);

	return 0;
}
DEFINE_SIMPLE_ATTRIBUTE(fake_cap_en_fops, fake_enable_show,
									fake_enable_store, "%llu\n");
#endif
...
#ifdef CONFIG_MACH_ACER_A9
	debugfs_create_file("FAKE_CAP", 0664, chip->dent, NULL,
			    &fake_cap_fops);
	debugfs_create_file("FAKE_CAP_ENABLE", 0664, chip->dent, NULL,
			    &fake_cap_en_fops);
#endif
				...
				struct kobj_attribute *attr, char * buf)
{
	...
	if (pm8921_batt_gauge) {
		dfi_ver = pm8921_batt_gauge->get_firmware_version();
		ret = sprintf(buf, "%d\n", dfi_ver);
	} else {
		ret = sprintf(buf, "Unavailable\n");
	}
	return ret;
}
				...
				struct kobj_attribute *attr, char * buf)
{
	if (pm8921_batt_gauge) {
		unsigned int fac;
		fac = pm8921_batt_gauge->bq27520_std_data(0x12);
...
static ssize_t PC_show(struct kobject *kobj,
				struct kobj_attribute *attr, char * buf)
{
	if (pm8921_batt_gauge) {
				...
				struct kobj_attribute *attr, char * buf)
{
	if (pm8921_batt_gauge) {
		unsigned int rm;
		rm = pm8921_batt_gauge->bq27520_std_data(0x10);
		return sprintf(buf, "%d\n", rm*1000);
	} else {
...
{
	if (is_vbus_disable) {
		pm8921_charger_vbus_draw(vbus_draw_current);
		pr_info("set vbus draw to %dmA\n", vbus_draw_current);
		is_vbus_disable = false;
	} else {
		pr_err("unbalance use, must cat vbus_disable first\n");
	}
...
{
	if (is_vbus_disable)  {
		pr_info("vbus is already disabled\n");
	} else {
		pm_chg_iusbmax_get(the_chip, &vbus_draw_current);
		pm8921_charger_vbus_draw(0);
		is_vbus_disable = true;
		pr_info("store vbus draw %dmA\n", vbus_draw_current);
	}
				...
				struct kobj_attribute *attr, char * buf)
{
	if (is_chg_disable) {
		pm8921_charger_enable(true);
		pr_info("enable charger\n");
		is_chg_disable = false;
		power_supply_changed(&the_chip->usb_psy);
		power_supply_changed(&the_chip->dc_psy);
	} else {
		pr_err("unbalance use, must cat chg_disable first\n");
	}
...
{
	if (is_chg_disable) {
		pr_info("charger is already disabled\n");
	} else {
		pm8921_charger_enable(false);
...
static ssize_t ctl_fw_update_show(struct kobject *kobj,
				struct kobj_attribute *attr, char * buf)
{
	int rc = 0;
	int ret = 0;
	...
	if (pm8921_batt_gauge) {
		rc = pm8921_batt_gauge->update_firmware(true);
		ret = sprintf(buf, "%s\n", (rc>=0)?"PASS":"FAIL");
	} else {
		ret = sprintf(buf, "No Gauge\n");
	}

	return ret;
}

static ssize_t ctl_it_enable_show(struct kobject *kobj,
				struct kobj_attribute *attr, char * buf)
{
	int rc = 0;
	int ret = 0;
	...
	if (pm8921_batt_gauge) {
		rc = pm8921_batt_gauge->it_enable(true);  //IT_ENABLE
		ret = sprintf(buf, "%s\n", (rc)?"PASS":"FAIL");
	} else {
		ret = sprintf(buf, "No Gauge\n");
	}
...
static ssize_t ctl_debug_on_show(struct kobject *kobj,
				struct kobj_attribute *attr, char * buf)
{
	is_psy_debug_on = true;

	return sprintf(buf, "Psy Debug On\n");
}
...
debug_attr_show(ctl_fw_update);
debug_attr_show(ctl_it_enable);
debug_attr_show(ctl_debug_on);
...
static struct attribute * attrs[] = {
	...
	&ctl_fw_update_attr.attr,
	&ctl_it_enable_attr.attr,
	&ctl_debug_on_attr.attr,
	NULL,
};
...
#ifdef CONFIG_MACH_ACER_A9
	power_supply_changed(&chip->batt_psy);
	if (!delayed_work_pending(&chip->update_heartbeat_work)) {
		pr_info("heartbeat work failed, re-schedule it.\n");
		schedule_delayed_work(&chip->update_heartbeat_work,
		      round_jiffies_relative(msecs_to_jiffies
					     (chip->update_time)));
	}
...
#ifdef CONFIG_MACH_ACER_A9
	wake_lock_init(&chip->usbin_wake_lock, WAKE_LOCK_SUSPEND, "pm8921_usbin");
	INIT_DELAYED_WORK(&chip->batt_missing_work, batt_missing_worker);
#endif
...
#ifdef CONFIG_MACH_ACER_A9
	pm8921_chg_dbg_kobj = kobject_create_and_add("dev-info_battery", NULL);
	if (pm8921_chg_dbg_kobj == NULL) {
		pr_err("kobject_create_and_add dev-info_battery failed\n");
	}
	rc = sysfs_create_group(pm8921_chg_dbg_kobj, &dbg_attr_group);
	if (rc) {
