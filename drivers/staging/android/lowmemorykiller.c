...
#if defined(CONFIG_MACH_ACER_A9)
	if (array_size >= 2 && lowmem_adj[array_size - 2] > 1)
		min_adj = lowmem_adj[array_size - 2];
do_kill_again:
#endif
	...
	for_each_process(p) {
		...
		spin_lock_bh(&p->alloc_lock);
		mm = p->mm;
		sig = p->signal;
		if (!mm || !sig) {
			spin_unlock_bh(&p->alloc_lock);
			continue;
		}
		oom_adj = sig->oom_adj;
		if (oom_adj < min_adj) {
...
#if defined(CONFIG_MACH_ACER_A9)
	if (killed_num == MAX_KILLED_NUM && array_size >= 2) {
		if (min_adj > lowmem_adj[array_size - 2])
			min_adj = lowmem_adj[array_size - 2];
		else
			min_adj--;
		goto do_kill_again;
