...
bool_t PutNextCBusTransactionWrapper(cbus_req_t * pReq, int iLine)
{
	...
	CBUS_DEBUG_PRINT("MhlTx:%d PutNextCBusTransaction %02X %02X %02X depth:%d head: %d tail:%d\n", iLine, (int)pReq->command, (int)((MHL_MSC_MSG == pReq->command) ? pReq->payload_u.msgData[0] : pReq->offsetData)
			,
			(int)((MHL_MSC_MSG ==
			       pReq->command) ? pReq->payload_u.
			      msgData[1] : pReq->payload_u.msgData[0])
...
void Disable_Backdoor_Access(void)
{
...
void Tri_state_dongle_GPIO0(void)
{
...
int charger_check(uint8_t data1)
{
	if ((data1) == 0x00) {
