...
extern int is_wakeup_by_powerkey(void);
extern void panel_need_wait(void);
extern int wait_panel_ready(struct msm_fb_data_type *mfd);
#endif
...
static int msm_fb_ext_resume(struct device *dev)
{
	...
	pdata = (struct msm_fb_panel_data *)mfd->pdev->dev.platform_data;
#ifdef CONFIG_MACH_ACER_A9
	if (is_wakeup_by_powerkey() &&
		mfd->panel_info.type == MIPI_CMD_PANEL) {
		mfd->fast_resume_enable = true;
		ret = msm_fb_resume_sub(mfd);
	}
#endif
	if (mfd->panel_info.type == HDMI_PANEL ||
...
*/
			...
			if (mfd->panel_info.type == MIPI_CMD_PANEL)
				wait_panel_ready(mfd);

			mfd->fast_resume_enable = false;
#endif
		}
		break;
...
#if defined(CONFIG_FB_MSM_LOGO) && defined(CONFIG_MACH_ACER_A9)
	/* We need clear framebuffer before android boot animation show.
	 * boot_pan_count = 1, booting logo
	 * boot_pan_count = 2, booting animation(android)
	 */
	if (boot_pan_count < 2){
		boot_pan_count++;
		if (boot_pan_count == 2)
			memset((unsigned char *)info->screen_base, 0x00,
