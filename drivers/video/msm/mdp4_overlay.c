...
#ifdef CONFIG_MACH_ACER_A9
			/* Workaround to avoid dirty fb in HDMI
			 * If RGB1 is filled with solid color and the
			 * RGB2 is mixed, we also need to set RGB2 to output
			 * solid color.
			 * */

			if (pnum == 0x00) {
				tmp = inpdw(MDP_BASE + 0xD0000);
				if (tmp & 0x1) { //make sure dtv(hdmi) is enabled.
					rgb_src_format = inpdw(MDP_BASE + 0x50050);
					rgb_src_format |= MDP4_FORMAT_SOLID_FILL;

					tmp = inpdw(MDP_BASE + 0x50058);
					tmp &= ~(MDP4_OP_FLIP_LR + MDP4_OP_SCALEX_EN);
					tmp &= ~(MDP4_OP_FLIP_UD + MDP4_OP_SCALEY_EN);
					outpdw(MDP_BASE + 0x50058, tmp);
					outpdw(MDP_BASE + 0x50050, rgb_src_format);
					outpdw(MDP_BASE + 0x51008, constant_color);

					tmp = inpdw(MDP_BASE + 0x50008);
					outpdw(MDP_BASE + 0x50000, tmp);
					outpdw(MDP_BASE + 0x18000, BIT(5));
				}
			}
#endif
		}
...
#ifdef CONFIG_MACH_ACER_A9
#define BLT_THRESHOLD	800
static u32 mdp4_large_downscale_check(struct mdp_overlay *req)
{
	int diff;

	if (!req)
		return 0;
	diff = req->src_rect.h - req->dst_rect.h;
	if ((diff > 0) && (diff > BLT_THRESHOLD))
		return 1;
	diff = req->src_rect.w - req->dst_rect.w;
	if ((diff > 0) && (diff > BLT_THRESHOLD))
		return 1;
	return 0;
}
...
#ifdef CONFIG_MACH_ACER_A9
		if (!use_blt)
			use_blt = mdp4_large_downscale_check(req);
#endif
